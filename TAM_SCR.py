...
# ----------------------------------------------------------
all_npy_exist = (
    os.path.exists(output_npy_TAM) and
    os.path.exists(output_npy_SCR) and
    os.path.exists(output_npy_params)
)

if all_npy_exist:
    print("\n=== All output .npy files already exist ===")
    print("Skipping data processing, loading existing data to generate PNG files...\n")
    
    # Load existing data
    max_time = np.load(output_npy_TAM)
    scr_array = np.load(output_npy_SCR)
    params = np.load(output_npy_params)
    min_x, min_y, dx, dy, Nx_global, Ny_global, T_global = params
    Nx_global = int(Nx_global)
    Ny_global = int(Ny_global)
    T_global = int(T_global)
    
    print(f"Loaded arrays with shape: TAM {max_time.shape}, SCR {scr_array.shape}")
    print(f"Grid: Nx={Nx_global}, Ny={Ny_global}, T={T_global}")
    print(f"X range: {min_x} → {min_x + dx * (Nx_global - 1)}")
    print(f"Y range: {min_y} → {min_y + dy * (Ny_global - 1)}")
else:
    print("\n=== Processing data from VTR files ===")

# ----------------------------------------------------------
# Collect Level3 files
# ----------------------------------------------------------
if not all_npy_exist:
...
# Clip values > 1e6 for visualization
max_time_clipped = np.clip(max_time, None, 1.0e6)
scr_array_clipped = np.clip(scr_array, None, 1.0e6)
